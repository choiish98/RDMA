...
struct rdma_cm_id *c_cm_id[NUM_QUEUES];
int queue_ctr = 0;
...
static int on_addr_resolved(struct rdma_cm_id *id)
{
	...
	TEST_NZ(rdma_create_queue(q, c_cc[queue_ctr++]));
...
//		return 1;
	...
	TEST_NZ(rdma_recv_wr(&client_session->queues[queue_ctr-1], &mr));
	TEST_NZ(rdma_poll_cq(client_session->queues[queue_ctr-1].cq, 1));
	return 1;
}
...
int start_rdma_client(struct sockaddr_in *s_addr)
{
	...
	for (unsigned int i = 0; i < NUM_QUEUES; i++) {
		...
		TEST_NZ((c_ec[i] == NULL));
		TEST_NZ(rdma_create_id(c_ec[i], &c_cm_id[i], NULL, RDMA_PS_TCP));
		TEST_NZ(rdma_resolve_addr(c_cm_id[i], NULL, (struct sockaddr *) s_addr, 2000));
